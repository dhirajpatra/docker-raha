...
if [ -z "${MYSQL_ROOT_PASSWORD_SET}" ]; then
    ...
    echo "    docker run -e MYSQL_ROOT_PASSWORD=<mysecret> ... dhirajpatra/raha:latest ..."
...
if find /var/www/raha/config -mindepth 1 | read; then
   echo "Configuration is mounted. Skipping copy ..."
else
   echo "First configuration. Copying config files ..."
   cp -R /var/www/raha/config.package/* /var/www/raha/config
   chown -R www-data:www-data /var/www/raha/config
fi
...
# Create database
if ! echo "USE raha;" | mysql &> /dev/null; then
    echo "Creating raha database ..."
    echo "CREATE DATABASE raha;" | mysql
else
...
if echo "SELECT COUNT(DISTINCT table_name) FROM information_schema.columns WHERE table_schema = 'raha';" | mysql | grep 0 &> /dev/null; then

    echo "Database is empty, installing Raha for the first time ..."

    # Create standard user and grant permissions
    MYSQL_USER_PASSWORD=$(openssl rand -base64 32)

    if ! echo "SELECT COUNT(*) FROM mysql.user WHERE user = 'raha';" | mysql | grep 1 &> /dev/null; then

        echo "Creating raha database user ..."

        echo "CREATE USER 'raha'@'localhost' IDENTIFIED BY '${MYSQL_USER_PASSWORD}';
              GRANT ALL PRIVILEGES ON raha.* TO 'raha'@'localhost';
              FLUSH PRIVILEGES;" | mysql
    else
        echo "Raha not installed but user was created. Resetting password ..."

        echo "ALTER USER 'raha'@'localhost' IDENTIFIED BY '${MYSQL_USER_PASSWORD}';
              FLUSH PRIVILEGES;" | mysql
    fi
    ...
    # We could use the following command to install Raha, but it could
    # imply:
    #
    ...
    #
    # Both are ugly. A better approach could be to make Raha store the
    ...
    echo "Raha already installed. Continue ..."
fi
